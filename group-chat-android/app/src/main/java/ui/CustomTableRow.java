...
import co.realtime.storagegroupchat.ChatRoomActivity;
import co.realtime.storagegroupchat.MessageActivity;
import co.realtime.storagegroupchat.R;
...
public class CustomTableRow extends TableRow {
		...
		addBt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (!newChannel.getText().toString().equals("")) {
					StorageHandler.addChannel(newChannel.getText()
                            .toString());
					ChatRoomActivity chatController = (ChatRoomActivity) context;
	...
	private void setUnread(View tr) {
		...
		Channel channelRef = StorageHandler.selfHandler.messages.get(content);
